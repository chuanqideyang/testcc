...
namespace basefw
{
    namespace quic
    {
        ...
        namespace
        {
            ...
            const int64_t kInitialRttMs = 1000;
...
// Updates the RTT based on a new sample.
                ...
                QuicTime now)
        {
            ...
            {
                SPDLOG_WARN("Ignoring measured send_delta, because it's is "
            ...
            else
            {
                ...
                SPDLOG_DEBUG(" smoothed_rtt(us): {} , mean_deviation(us): {}",
                        smoothed_rtt_.ToMicroseconds(), mean_deviation_.ToMicroseconds());
            }
            return true;
        }
        ...
        QuicTime::Delta RttStats::GetStandardOrMeanDeviation() const
        {
            if (!calculate_standard_deviation_)
            {
                SPDLOG_ERROR("calculate_standard_deviation_ = {}", calculate_standard_deviation_);
            }
        ...
        {
            if (!has_valid_standard_deviation)
            {
                SPDLOG_ERROR("has_valid_standard_deviation = {}", has_valid_standard_deviation);
            }
